...
Если вы немного подзабыли, что всё это значит, то обратитесь к [описанию в курсе BGO](https://github.com/netology-code/bgo-homeworks/blob/master/12_docker/packages.md).

### Задача

Необходимо реализовать вычислительный кластер с балансировкой запросов на базе Kubernetes.

Для этого мы с создадим Stateless-сервисы, задача которых – выполнять вычисления (а значит, масштабироваться они должны хорошо).
...
Итак, клиент вам присылает JSON с массивом транзакций. А вы ему отвечаете:
```json
{
  "worker_id": "uuid-here",
  "data": [
...
Где:
1. `worker_id` – ad-hoc идентификатор конкретного сервера, который производил вычисления (мы его сгенерируем программно, поскольку только начали знакомиться с Kubernetes).
...
Kubernetes, как и GitHub, умеет хранить секреты. В качестве секрета будут выступать наши credentials к GitHub Packages.
...
**Важно**: мы специально не делаем секреты в виде yaml-файлов, потому что студенты часто коммитят их прямо в GitHub (что не очень хорошо, поскольку там ваш токен в открытом виде*).
...
Шаблон взят из лекции. Мы разворачиваем 3 реплики, взяв образ из нашего реестра образов (вы можете заменить `replica` на своё имя).

Главное: именно эта строка отвечает за то, какой секрет мы используем для получения образов:
```yaml
      imagePullSecrets:
        - name: github-packages
```
